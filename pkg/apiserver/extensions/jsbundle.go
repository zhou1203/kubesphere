...
func NewJSBundle(cache cache.Cache) filters.Middleware {
	return &jsBundle{cache: cache}
}

func (s *jsBundle) Handle(w http.ResponseWriter, req *http.Request) bool {
	...
	if err := s.cache.List(req.Context(), &jsBundles, &client.ListOptions{}); err != nil {
		message := fmt.Errorf("failed to list jsBundles")
